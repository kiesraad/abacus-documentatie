# Documentatie Abacus

Hier vind je de documentatie over Abacus. De documentatie wordt telkens aangevuld en bijgewerkt.

## Gebruikersdocumentatie

Voor informatie over het installeren, starten en gebruiken van Abacus ga je naar de website [Abacus Documentatie](https://kiesraad.github.io/abacus-documentatie/). Hier vind je niet alleen alle beschikbare gebruikersdocumentatie, maar bieden we ook uitleg over het verkiezingsproces.

De brondocumenten van de gebruikersdocumentatie zijn te vinden in deze repository in de map [gebruikersdocumentatie](https://github.com/kiesraad/abacus-documentatie/tree/src/main/gebruikersdocumentatie).

## Ontwikkeling

- Functionaliteit:
  - [Eisen en wensen voor functionaliteit](./functionaliteit/functionaliteit-eisen-en-wensen.md)
- Ontwikkelproces:
  - [Werkwijze voor GitHub](./ontwikkelproces/GitHub-werkwijze.md)
  - [Proces voor ontwikkeling en releases](./ontwikkelproces/proces-ontwikkeling-en-releases.md)
  - [Uitgangspunten voor testen en kwaliteit](./ontwikkelproces/testen-en-kwaliteit.md)
- Referentie:
  - [Afkortingenlijst](./referentie/afkortingenlijst.md)
  - [Woordenlijst Nederlands - Engels](./referentie/woordenlijst-NL-EN.md)
- Softwarearchitectuur:
  - [Overwegingen van talen en frameworks](./softwarearchitectuur/overwegingen-talen-en-frameworks.md)
  - [Overzicht van de softwarearchitectuur](./softwarearchitectuur/Overzicht.md)
- Use cases:
  - [Readme usecases](./use-cases/README.md)
  - [Autorisatiematrix Gemeenteraadsverkiezingen](./use-cases/autorisatiematrix.md)
  - [Validatieregels en plausibiliteitschecks voor invoer tellingen](./use-cases/validatieregels-plausibiliteitschecks-tellingen.md)
  - [Input- en output-bestanden](./use-cases/input-en-output/input-output-bestanden.md)
- Verkiezingsproces:
  - [Verloop van verkiezingen](./verkiezingsproces/verloop-verkiezingen.md)
  - [Wie doet welke stembureaus](./verkiezingsproces/wie-doet-welke-stembureaus.md)
  - [Zetelverdeling voor gemeenteraadsverkiezingen](./verkiezingsproces/zetelverdeling-GR.md)

## Testdocumenten

In deze repository staan verschillende testdocumenten:

- In de map [test-pvs-juinen](https://github.com/kiesraad/abacus-documentatie/tree/main/gebruikersdocumentatie/src/testdocumenten/test-pvs-juinen) staan de processen-verbaal voor de testgemeente Juinen, waarmee je de testverkiezing kunt invoeren.
- Daarnaast staan in de map [test-emls-juinen](https://github.com/kiesraad/abacus-documentatie/tree/main/gebruikersdocumentatie/src/testdocumenten/test-emls-juinen) de eml-bestanden waarmee je deze testverkiezing zelf kunt toevoegen.

## Externe documentatie

- [Formele beschrijving van de berekening van de zetelverdeling - kiesraad.nl](https://www.kiesraad.nl/adviezen-en-publicaties/formulieren/2016/osv/osv-bestanden/formele-beschrijving-berekening-zetelverdeling)

## Bijdragen

Het kan zijn dat sommige informatie niet up-to-date is i.v.m. wijzigingen in de Kieswet, de Kiesregeling of het Kiesbesluit. Mocht je fouten of onvolledigheden vinden, dan kun je dit doorgeven in een issue.
