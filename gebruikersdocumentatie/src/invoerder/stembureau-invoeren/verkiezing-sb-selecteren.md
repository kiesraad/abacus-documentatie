# Verkiezing en stembureau selecteren

Selecteer eerst de verkiezing waarvoor je stemmen wil invoeren. Hier zie je ook wat de status van de verkiezing is.

![Verkiezing selecteren](./img/inv-verkiezing-selecteren.png)

Selecteer nu het stembureau.

## Gemeentelijk stembureau

- Voer het nummer in van het stembureau dat je wil invoeren en selecteer **Beginnen**. Het nummer vind je op het proces-verbaal.
- Weet je het nummer niet, selecteer dan onderaan de pagina **Toon de lijst met nog in te vullen stembureaus** en selecteer vervolgens het juiste stembureau.

![Stembureau selecteren - gemeentelijk stembureau](./img/inv-sb-selecteren-gsb.png)

## Centraal stembureau

- Je kunt het stembureau meteen selecteren. Abacus stuurt je dan direct door naar de stap [Aantal kiezers en stemmen](./kiezers-stemmen.md).

![Stembureau selecteren - centraal stembureau](./img/inv-sb-selecteren-csb.png)
